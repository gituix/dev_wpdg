...
class EM_Widget extends WP_Widget {
    ...
    function widget($args, $instance) {
    	...
    	if( !empty($instance['title']) ){
		    ...
		    echo '<div style="background-color:#D00; letter-spacing: 3px; text-align:center; font-size:14px; color:#fff";>'.apply_filters('widget_title',$instance['title'], $instance, $this->id_base).'</div>';
		...
		if ( count($events) > 0 ){
			foreach($events as $event){				
				if( $li_wrap ){
					echo '<div class=event>'. $event->output($instance['format']) .'</div>';
				}else{
